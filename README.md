...
三端共用一份关注列表：网页版和 Bot 天然同账号；桌面端在「设置 → 网页 / Bot 账号」里粘一次绑定链接，之后关注和房产也跟着走，以云端为准。
...
- **房区图**：常显的房区地图，鼠标滑过房屋条目（手机上点）即高亮对应地块，
  看得出这套房在小区里的具体位置和邻居。5 个房区 × 主城区（1-30 号）/ 扩建区（31-60 号）
...
| 报名提醒 | 申请期截止前（计划抽的房） |
| 开奖提醒 | 进入公示期（已报名的房） |
| 确认归属 | 公示期截止前，中签要去点门牌购入，逾期扣 50% 申请金 |
| 抽签金返还 | 公示期结束后 90 天内要点门牌确认，否则金币不返还 |
| 下轮开抽 | 准备期结束 |
...
房子已经炸了可以标记，转成 35 天资产回收倒计时（家具庭具 + 购地金币的 80%）。

**提醒渠道**：Telegram Bot / Bark（iOS）/ WxPusher APP / Windows 通知（桌面端）
